...
import { ExecutorServicePageExposeFunc } from '@/services/executor/type/page/expose'
...
const SelectorMaskId = 'mama-mia-selector'

const setMaskOpacity = async (page: Page, opacity: number) => {
  await page.evaluateHandle(
    (opacity: number, maskId) => {
...
export const usePageDomSelector = async (
  ...
  const injectHelperCodeToPage = async (page: Page) => {
    await page.exposeFunction(
      ExecutorServicePageExposeFunc.onSelectorHelperChoose,
    ...
    await page.evaluateOnNewDocument(
      (maskId, callbackFuncName) => {
        ...
        document.addEventListener('click', (e) => {
          ...
          ;(window as any)[callbackFuncName](getChooseElementDetail(e))
        })
      },
      SelectorMaskId,
      ExecutorServicePageExposeFunc.onSelectorHelperChoose
